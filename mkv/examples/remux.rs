...
use mkv::{ElementRead, ElementWrite};
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let size = meta.len();
    debug!("size {size}");
    let mut input = tokio::fs::File::open(input).await.context(format!("Failed open '{input}'"))?;
    ...
    let (header, _) = mkv::structs::EbmlHeader::read(&mut input).await.context("Failed EbmlHeader::read")?;
    debug!("header {:#?}", header);
    // debug!("header write {len} bytes");

    let size = mkv::structs::Segment::read_header(&mut input).await.context("Failed Segment::read_header")?;
    debug!("Segment size {size:?}");

    let (seek_head, _) = mkv::structs::SeekHead::read(&mut input).await.context("Failed SeekHead::read")?;
    // debug!("seek_head {seek_head:#?}");
    // debug!("seek_head write {len} bytes");

    let void = mkv::element::Element::read_header(&mut input).await.context("Failed void Element::read")?;
    void.skip(&mut input).await.context("Failed skip void")?;
    // debug!("void {void:#?}");
    let (info, _) = mkv::structs::Info::read(&mut input).await.context("Failed Info::read")?;
    // debug!("info {info:#?}");

    let (tracks, _) = mkv::structs::Tracks::read(&mut input).await.context("Failed Tracks::read")?;
    // debug!("tracks {tracks:?}");
    ...
    let (attachments, _) = mkv::structs::Attachments::read(&mut input).await.context("Failed Attachments::read")?;
    debug!("attachments {:?}", attachments.attached_file.len());
    // attachments.write(&mut output).context("Failed Attachments::write")?;

    let mut output = tokio::fs::File::create(output).await.context(format!("Failed to create '{output}'"))?;
    header.write(&mut output).await.context("Failed EbmlHeader::write")?;
    ...
    for _ in 0..100 {
        ...
        for _ in 0..10 {
            let (mut cluster, _) = mkv::structs::Cluster::read(&mut input).await.context("Failed Cluster::read")?;
        ...
        segment.write(&mut output).await.context("Failed Segment::write")?;
